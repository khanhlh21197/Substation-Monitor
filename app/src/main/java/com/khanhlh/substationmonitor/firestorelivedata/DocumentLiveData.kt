...
class DocumentLiveData<T : FirestoreModel>(
    ...
    override fun onActive() {
        ...
        reference.addSnapshotListener { documentSnapshot, firebaseFirestoreException ->
            if (firebaseFirestoreException != null) {
                ...
                postValue(
                    FirestoreResource.success(
                        documentSnapshot?.toObject(modelClass)?.apply { id = documentSnapshot.id })
                )
            }
        }
    }
     ...
     * @return A [TaskLiveData] representing the state of the operation, as a [TaskResult].
     */
    fun update(field: String, value: Any): TaskLiveData<Void> =
        reference.update(field, value).asLiveData()

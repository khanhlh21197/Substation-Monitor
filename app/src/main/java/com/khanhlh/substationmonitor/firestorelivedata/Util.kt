package com.khanhlh.substationmonitor.firestorelivedata
 ...
 */
inline fun <reified T : FirestoreModel> CollectionReference.observe(
    lifecycleOwner: LifecycleOwner,
    observer: Observer<FirestoreResource<List<T>>>
) {
 ...
 */
inline fun <reified T : FirestoreModel> Query.observe(
    lifecycleOwner: LifecycleOwner,
    observer: Observer<FirestoreResource<List<T>>>
) {
    this.asLiveData<T>().observe(lifecycleOwner, observer)
 ...
 */
inline fun <reified T : FirestoreModel> DocumentReference.observe(
    lifecycleOwner: LifecycleOwner,
    observer: Observer<FirestoreResource<T>>
) {
    this.asLiveData<T>().observe(lifecycleOwner, observer)
}